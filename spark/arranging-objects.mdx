...
Only the active setup's rows carry write controls (visibility, delete, revert). Moving or removing an object while a non-base setup is active overrides it for that setup only — the Base setup and other setups keep their own version. See [setups and cameras](/spark/setups-and-cameras) for how setups and the active setup work.
